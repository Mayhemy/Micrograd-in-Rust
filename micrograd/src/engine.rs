		...
		mod validator;
		use validator::MicrogradValidator;
		...
		impl ValueWrapper{
    		...
    		fn topo_recursive(&self, topologically_sorted : &mut Vec<ValueWrapper>, visited: &mut HashSet<ValueWrapper>){
    			if !visited.contains(self){
        			visited.insert(self.clone());
        			for single_vertex in self.0.borrow().prev.iter(){
            			let upcast = ValueWrapper(single_vertex.clone());
            			upcast.topo_recursive(topologically_sorted, visited);
        			}
		...
		impl Mul for ValueWrapper{
    		...
    		fn mul(self, other: Self) -> Self{
        		...
        		let result_reference = result.0.clone();
        		result.0.borrow_mut().backward = Some(Box::new(move || {
        			let left_data = left_val.borrow().data;
        			let right_data = right_val.borrow().data;
        			let result_grad = result_reference.borrow().grad;
            		left_val.borrow_mut().grad += right_data * result_grad;
            		right_val.borrow_mut().grad += left_data * result_grad;
        		}));
		...
		fn main() {
			let validator = MicrogradValidator::minimal_new();
    		validator.validate_all_test();
		...
		fn gradient_descent(neural_network: &MLP, learning_rate : f64){
			let parameters = neural_network.parameters();
			for parameter in &parameters{
        		let grad = parameter.0.borrow().grad;
        		parameter.0.borrow_mut().data -= learning_rate * grad;
			}
		}
		...
		pub fn calculate_loss(prediction_values: Vec<ValueWrapper>, target_values: &Vec<ValueWrapper>) -> ValueWrapper{
